...
void eg_picker_init(
  ...
  re_canvas_init(&picker->canvas, width, height, VK_FORMAT_R32_UINT);
  picker->canvas.clear_color = (VkClearColorValue){
  ...
  re_buffer_init(
      &picker->pixel_buffer, &(re_buffer_options_t){
...
eg_cmd_info_t eg_picker_begin(eg_picker_t *picker) {
  ...
  re_begin_cmd_buffer(
      picker->cmd_buffer, &(re_cmd_buffer_begin_info_t){
...
uint32_t eg_picker_get(eg_picker_t *picker, uint32_t x, uint32_t y) {
  re_image_transfer_to_buffer(
      picker->canvas.resources[0].color.image, &picker->pixel_buffer,
      g_ctx.transient_command_pool, x, y, 1, 1, 0, 0);
