...
Camera::~Camera() {
  if (*this) {
    ...
    auto descriptorPool = renderer::ctx().m_descriptorManager.getPool(renderer::DESC_CAMERA);
...
void Camera::bind(renderer::Window &window, renderer::GraphicsPipeline &pipeline) {
  auto i = window.getCurrentFrameIndex();
