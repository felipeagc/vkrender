...
inline bool fstd_bitset_at(uint8_t *bitset, uint32_t pos) {
...
inline void fstd_bitset_set(uint8_t *bitset, uint32_t pos, bool val) {
  uint32_t index = pos / 8;
  uint32_t bit = pos % 8;
...
inline void fstd_bitset_reset(uint8_t *bitset, uint32_t size) {
